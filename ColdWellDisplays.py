...
lPath = Path(os.getcwd()+"/list.txt")
with open(str(lPath)) as f:
    ips = f.readlines()
ips_list = []
if len(ips) > 0:
    for ip in ips:
...
def images():
    ...
    while (run == True):
        slideshow()
xi = threading.Thread(target=images)

tk.Spinbox(canvas, from_=1, to=100,textvariable=secs).grid()
tk.Button(canvas, text="Play Slide Show", command=xi.start).grid()
def pause(): return stop()

tk.Button(window, text="stop", command=pause()).grid()
